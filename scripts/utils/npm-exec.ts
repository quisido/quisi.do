...
export default async function npmExec(...script: string[]): Promise<string> {
  globalThis.console.log(' ');
  globalThis.console.log('-'.repeat(MAX_CONSOLE_LENGTH));
  globalThis.console.log(' ');
  logCommand('npm', ...script);

  try {
    return await spawnAsync(FILE, [...ARGS, ...script]);
  } catch (err: unknown) {
    return handleNpmExecError(err, script);
  }
}
