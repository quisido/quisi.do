...
import FetchOperation from './features/fetch-operation.js';
  ...
  async fetch(
    ...
    try {
      ...
      if (isAnaylticsEngineDataset(ANALYTICS)) {
        ...
        throw operation.createError(
          'The analytics engine dataset is missing.',
          {
      ...
      if (ENVIRONMENT === Environment.Development || isAwaitExperiment) {
        ...
        operation.emit(MetricName.AwaitExperiment);
      } else {
        ctx.waitUntil(
          optionalPromise
            .then((): void => {
