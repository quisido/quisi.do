...
describe('handleFetchError', (): void => {
  it('should emit, log, and respond', (): void => {
    const { headers, status } = handleFetchError.call(
      Object.assign(
        new Worker({
          invalidPrivateDatasetMetricName: '_',
          invalidPublicDatasetMetricName: '_',
    ...
    expect(TEST_PUBLIC_METRIC_EMITTER).toHaveBeenLastCalledWith({
      name: MetricName.UnknownError,
    });
    expect(TEST_PRIVATE_ERROR_LOGGER).toHaveBeenCalledTimes(ONCE);
    expect(TEST_PRIVATE_ERROR_LOGGER).toHaveBeenLastCalledWith(
